...
export function Home() {
  return (
    <div className="min-h-[calc(100vh-8rem)]">
      ...
      {/* Quick links */}
      <section>
        <h2 className="mb-6 text-lg font-semibold text-slate-700 dark:text-white">바로가기</h2>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {features.map((f) => (
            <Link
              key={f.to}
              to={f.to}
              className="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:border-slate-300 hover:shadow-md dark:border-zinc-700 dark:bg-zinc-900 dark:hover:border-zinc-600"
            >
              <div
              ...
              />
              <div className="relative">
                <h3 className="font-semibold text-slate-800 group-hover:text-slate-900 dark:text-white dark:group-hover:text-white">{f.title}</h3>
                <p className="mt-1.5 text-sm leading-relaxed text-slate-500 dark:text-slate-300">{f.description}</p>
                <span className="mt-3 inline-block text-sm font-medium text-slate-600 group-hover:text-slate-800 dark:text-slate-400 dark:group-hover:text-white">
                  이동 →
                </span>
              </div>
            </Link>
      ...
      {/* Footer hint */}
      <p className="mt-12 text-center text-sm text-slate-400 dark:text-slate-400">
