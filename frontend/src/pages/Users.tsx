...
export function Users() {
  ...
  return (
    <div>
      <h1 className="mb-6 text-2xl font-bold text-slate-800">사용자</h1>

      <div className="grid gap-8 lg:grid-cols-2">
        <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-lg font-semibold text-slate-800">사용자 생성</h2>
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-slate-600">역할</label>
              <select
          ...
          {created && (
            <div className="mt-4 rounded-lg bg-slate-100 p-3 text-sm">
              <p className="font-medium text-slate-700">생성됨</p>
              <p>ID: {created.id}, 이름: {created.name}, 역할: {created.role}</p>
            </div>
          )}
        </section>

        <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-lg font-semibold text-slate-800">사용자 ID 조회</h2>
          ...
          {!listLoading && userList.length === 0 && nameSearch.trim() !== '' && (
            <p className="mt-3 text-slate-500">검색 결과가 없습니다.</p>
          )}
          {!listLoading && userList.length === 0 && nameSearch.trim() === '' && (
            <p className="mt-3 text-slate-500">등록된 사용자가 없습니다. 사용자 생성을 먼저 해 주세요.</p>
          )}
        </section>
      </div>
    </div>
