...
export function ProblemCard({
  ...
  const choiceStyle = (key: string) => {
    ...
    return 'bg-slate-50/50 border-slate-200 dark:bg-slate-800/50 dark:border-slate-600';
  };

  return (
    <div className="rounded-xl border border-slate-200/80 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        ...
        {problem.tags?.length ? (
          <span className="flex flex-wrap gap-1.5">
            {problem.tags.map((tag) => (
              <span
                key={tag}
                className="inline-flex rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-medium text-slate-600 dark:bg-slate-700 dark:text-slate-300"
              >
                {tag}
              </span>
            ))}
